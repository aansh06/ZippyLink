...
const Navbar = () => {
  ...
  return (
    <div className="h-16 bg-gradient-to-r from-indigo-700 via-purple-700 to-violet-700 z-50 flex items-center sticky top-0">
      <div className="lg:px-14 sm:px-8 px-4 w-full flex justify-between">
        <Link to="/">
          <h1 className="font-bold text-3xl text-white italic sm:mt-0 mt-2">
            ZippyLink
        ...
        <ul
          className={`flex sm:gap-10 gap-4 sm:items-center sm:mt-1 sm:pt-0 pt-3 text-slate-200 sm:static absolute left-0 top-[62px] sm:shadow-none shadow-md ${
            navbarOpen ? "h-fit sm:pb-0 pb-5" : "h-0 overflow-hidden"
          } transition-all duration-100 sm:h-fit sm:bg-none bg-gradient-to-b from-indigo-700 via-purple-700 to-violet-700 sm:w-fit w-full sm:flex-row flex-col px-4 sm:px-0`}
        >
          <li className="hover:text-violet-300 font-[500] transition-all duration-150">
            <Link
              className={`${
                path === "/" ? "text-white font-semibold" : "text-slate-300"
              }`}
              to="/"
            >
              Home
          ...
          <li className="hover:text-violet-300 font-[500] transition-all duration-150">
            <Link
              className={`${
                path === "/about" ? "text-white font-semibold" : "text-slate-300"
              }`}
              to="/about"
            >
              About
          ...
          {token && (
            <li className="hover:text-violet-300 font-[500] transition-all duration-150">
              <Link
                className={`${
                  path === "/dashboard" ? "text-white font-semibold" : "text-slate-300"
                }`}
                to="/dashboard"
              >
                Dashboard
          ...
          {!token && (
            <Link to="/register">
              <li className="sm:ml-0 -ml-1 bg-violet-600 text-white cursor-pointer w-24 text-center font-semibold px-2 py-2 rounded-md hover:bg-violet-500 transition-all duration-150">
                SignUp
              </li>
            </Link>
          )}
          {token && (
            <button
              onClick={onLogOutHandler}
              className="sm:ml-0 -ml-1 bg-rose-600 text-white cursor-pointer w-24 text-center font-semibold px-2 py-2 rounded-md hover:bg-rose-500 transition-all duration-150"
            >
              LogOut
            </button>
          )}
